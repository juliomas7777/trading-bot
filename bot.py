...
async def main():
    ...
    logger.info("🚀 Bot Sniper Iniciado (Armónicos + Velas + RSI 80/20)")
                ...
                for cat, symbols in ASSETS.items():
                    for sym in symbols:
                        try:
                            ...
                            if armonico:
                                # 3. Confirmar Acción del Precio (Velas)
                                confirmacion = confirmar_vela(df, armonico["dir"])
                                
                                # 4. Validación estricta del RSI (80/20)
                                rsi_valido = False
                                if "ALZA" in armonico["dir"] and rsi_actual <= 20:
                                    rsi_valido = True
                                    estado_rsi = "🔵 SOBREVENTA"
                                elif "BAJA" in armonico["dir"] and rsi_actual >= 80:
                                    rsi_valido = True
                                    estado_rsi = "🔴 SOBRECOMPRA"
                                
                                # 🔥 SOLO SI SE CUMPLEN LAS 3 COSAS ENVÍA LA SEÑAL
                                if confirmacion and rsi_valido:
                                    id_s = f"{sym}_{tf}_{hora_ref.strftime('%H%M')}"
                                    if id_s not in registro_velas:
                                        nombre_bot = TRADUCCION_NOMBRES.get(sym, sym)
                                        msg = f"🎯 *SEÑAL SNIPER:* {nombre_bot}\n"
                                        msg += f"🕒 TF: *{tf}*\n"
                                        msg += "━━━━━━━━━━━━━━━━━━\n"
                                        msg += f"📐 Armónico: *{armonico['nombre']}*\n"
                                        msg += f"🕯️ Vela: *{confirmacion}*\n"
                                        msg += f"📊 RSI: *{rsi_actual:.1f}* ({estado_rsi})\n"
